...
public partial class AddEditServicePageViewModel : BaseViewModel
{
    ...
    private async Task LoadIntakeFieldsAsync()
    {
        try
        {
    ...
    [RelayCommand]
    private async Task EditFieldAsync(IntakeFieldResponse? field)
    {
        try
        {
    ...
    [RelayCommand]
    private Task MoveFieldUpAsync(IntakeFieldResponse? field) => MoveFieldAsync(field, -1);

    [RelayCommand]
    private Task MoveFieldDownAsync(IntakeFieldResponse? field) => MoveFieldAsync(field, 1);

    // Order is what the customer sees, so it is stored rather than implied: the two rows swap
    // sort_order and both are written.
    private async Task MoveFieldAsync(IntakeFieldResponse? field, int direction)
    {
        try
        {
            if (field is null)
