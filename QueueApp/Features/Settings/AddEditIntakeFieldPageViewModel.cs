...
public partial class AddEditIntakeFieldPageViewModel : BaseViewModel
{
    ...
    private void Apply(IntakeFieldResponse? field)
    {
        if (field is null)
            return;
    ...
    [RelayCommand]
    private void SelectType(IntakeTypeOption? option)
    {
        try
        {
            if (option is null)
    ...
    [RelayCommand]
    private void RemoveOption(IntakeOptionRow? option)
    {
        try
        {
            if (option is null)
    ...
    private void RaiseOptionState() => OnPropertyChanged(nameof(NeedsMoreOptions));
}
